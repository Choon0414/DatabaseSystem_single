...
void PerformanceTester::printAvgResult() const {
    cout << fixed << setprecision(6);
    cout << "[ 읽기 + 조인 + 출력]" << endl;
    cout << "● 총 처리 시간: " << (loadTime + outputTime + (joinTime/6)) <<  " 초" << endl;
    cout << "● 평균 CPU 사용률: " << ((loadCpuUsage + outputCpuUsage + joinCpuUsage)/6) << " %" << endl;
...
void PerformanceTester::printStatus() {
    ...
    printResult("조인", (joinTime/6), getJoinCPUUsage(), getJoinMemoryUsage());
